...
1. 使用登入按鈕
    - 未登入：一般用戶購買
    - 以登入：多了儀表板、商品管理，訂單管理(訂單可以多張，用戶可以一直填寫送出)

## promt

我想建立一個專門銷售咖啡豆的網站，請依照下述需求幫我設計一個靜態網站

1. 第一步是提供給我風格外觀圖檔，並詢問我意見
    1. 網頁架構：
        - 前台
            - 首頁(`/home`)
            - 產品列表頁(`/products`)
        ...
        - 後台
            - 商品管理(`/admin/products`)
            - 訂單管理(`/admin/order`)
            - 銷售儀錶板(`/admin/dashboard`)

    2. 網頁layout大致分為以下區塊：
        - header：有logo與登入管理後台的超連結
        - navbar：
            1. 首頁
            2. 關於我
            3. 產品
    ...
    3. 風格外觀請參考參考[網站](https://www.designprompts.dev/organic)，請參考prompt進行設計，但要優先維持前面兩點的功能完整性：
    <role>
